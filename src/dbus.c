...
#define cdbus_m_opts_get_do(tgt, apdarg_func) \
  ...
  cdbus_m_opts_get_do(inactive_dim_ms, cdbus_reply_int32);
  cdbus_m_opts_get_do(inactive_undim_ms, cdbus_reply_int32);
...
#define cdbus_m_opts_set_do(tgt, type, real_type) \
  ...
  // fade_in_ms
  if (!strcmp("fade_in_ms", target)) {
    int32_t val = 0;
    if (!cdbus_msg_get_arg(msg, 1, DBUS_TYPE_INT32, &val))
      return false;
    ps->o.fade_in_ms = max_i(val, 0);
    goto cdbus_process_opts_set_success;
  }

  // fade_out_ms
  if (!strcmp("fade_out_ms", target)) {
    int32_t val = 0;
    if (!cdbus_msg_get_arg(msg, 1, DBUS_TYPE_INT32, &val))
      return false;
    ps->o.fade_out_ms = max_i(val, 0);
    goto cdbus_process_opts_set_success;
  }
